...
console.log("test ", dateFormat("2022-02-02"))

var timeScale = d3.scaleTime()
  /* .domain([d3.min(taskArray, function (d) { console.log("start time ", d.startTime, "result ", dateFormat(d.startTime)); return dateFormat(d.startTime); }),
  d3.max(taskArray, function (d) { return dateFormat(d.endTime); })]) */
  .domain([
            d3.min(taskArray, function (d) { return dateFormat(d.startTime); }),
            d3.max(taskArray, function (d) { return dateFormat(d.endTime); })
          ])
  .range([0, w - 150]);
...
function makeGrid(theSidePad, theTopPad, w, h) {
  ...
  var xAxis = d3.axisBottom()
                .scale(timeScale)
                // .orient('bottom')
                // .ticks(d3.timeDays().every(1))
                // .ticks().every()
                .ticks(d3.timeDay.filter(d=>d3.timeDay.count(0, d) % 1 === 0))
